...
class GpuEye(GpuKernelBase, Op):
    ...
    def gpu_kernels(self, node, name):
        code = """
KERNEL void eye(GLOBAL_MEM %(ctype)s *a, ga_size n, ga_size m, ga_size k) {
    ga_int off = (ga_int) k;
    ga_int col_off = max(off, (ga_int)0);
    ga_int row_off = -min(off, (ga_int)0);
    ...
    for (ga_size i = LID_0; i < nb; i += LDIM_0) {
        ...
        return [Kernel(
                code=code, name="eye",
                params=[gpuarray.GpuArray, gpuarray.SIZE, gpuarray.SIZE, gpuarray.SIZE],
